...
import uk.gov.digital.ho.hocs.house.dto.HouseRecord;
...
public class HouseResource {
    ...
    Set<House> houses = houseService.getAllHouses();
        List<DataListEntityRecord> dataListEntityRecords = houses.stream().map(this::translateToDataListRecord).flatMap(Collection::stream).collect(Collectors.toList());
    ...
    private List<DataListEntityRecord> translateToDataListRecord(House house){
        return house.getMembers().stream().map(m -> DataListEntityRecord.create(m, house.getName())).collect(Collectors.toList());
    }

...
curl -u $user:$pass -X POST $target_uri/users/dept/CD -F "file=@$data_dir/users/CD.csv" \
 -H "Content-Type: multipart/form-data"
curl -u $user:$pass -X POST $target_uri/users/dept/IIG -F "file=@$data_dir/users/IIG.csv" \
 -H "Content-Type: multipart/form-data"
curl -u $user:$pass -X POST $target_uri/users/dept/Press_Office -F "file=@$data_dir/users/Press_Office.csv" \
 -H "Content-Type: multipart/form-data"
curl -u $user:$pass -X POST $target_uri/users/dept/Ops_Users -F "file=@$data_dir/users/Ops_Users.csv" \
 -H "Content-Type: multipart/form-data"
...
then
    ...
    curl -u $user:$pass $target_uri/users/dept/IIG/publish
    curl -u $user:$pass $target_uri/users/dept/Press_Office/publish
    curl -u $user:$pass $target_uri/users/dept/Ops_Users/publish
